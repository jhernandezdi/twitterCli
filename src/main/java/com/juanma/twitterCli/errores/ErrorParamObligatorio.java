
package com.juanma.twitterCli.errores;

import org.sine95.kernel.base.errores.IError;
import java.util.*;
import org.sine95.kernel.util.*;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.juanma.twitterCli.enums.EnumCategoriasErrores;

//Clase generada, no tocar

public class ErrorParamObligatorio implements IError
{

  protected Map<String,String> params=new HashMap<>();
  protected Set<EnumCategoriasErrores> categorias=new HashSet<>();

  private static Logger log = LoggerFactory.getLogger(ErrorParamObligatorio.class);
 
  //Code:SERVER_GEN_003
   
  public ErrorParamObligatorio(
    
   String nombre
  )
  {
    super();
    String _params_=UtilParams.paramsToString(
   "nombre", nombre
      
    );
    
    StackTraceElement[] stackTrace = Thread.currentThread().getStackTrace();
    if (log.isErrorEnabled()) {
	    log.error("Entrando en ErrorParamObligatorio ("+_params_+"):"+UtilString.toString(stackTrace));
    }
    

    if(nombre!=null)
      params.put("nombre",nombre.toString());
    else 
      params.put("nombre","<NULL>");
    
    



  }
  


  
  public void setParams(HashMap<String,String> params)
  {
  	this.params=params;
  }

  @Override
  public String getMensaje()
  {
    return UtilError.genMsg("Parametro obligatorio. El parametro ((nombre)) es obligatorio",getParams());  
  }

	public String getCode(){
    return "SERVER_GEN_003";
  }

	public Map<String, Object> getParams()
  {
    HashMap<String,Object> res= new HashMap<>();
    res.putAll(params);
    return res;
  }

  public boolean isPerteneceCategoria(EnumCategoriasErrores categ)
  {
    return categorias.contains(categ);
  }

  public Set<EnumCategoriasErrores> getCategorias()
  {
    Set<EnumCategoriasErrores> res=new HashSet<>();
    res.addAll(categorias);
    return res;
  }
  @Override
  public String toString()
  {
    return "Error:"+getCode()+"["+getMensaje()+"]";
  }
}

